...
const schooldata = async () => {
    ...
    try {
        const response = await axios.post('http://localhost:8080/admin/query_school', {});
...
const submitDelete = async (v) => {
    ...
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    try {
        const response = await axios.post('http://localhost:8080/admin/delete_school', data);
...
const submitAdd = async () => {
    ...
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    try {
        const response = await axios.post('http://localhost:8080/admin/add_school', {
            school:form1.school,
        });
        if (response.data.code === 20000) {
            ...
            <div class="m-4">
                ...
                <el-form :model="form1">
                    <el-form-item label="School" :label-width="formLabelWidth">
                        <el-input v-model="form1.school" autocomplete="off" />
                    </el-form-item>
                </el-form>
            </div>
        </div>
    ...
    <el-card class="page-container">
        <template #header>
            <div class="header">
                <span>学校</span>
                <div class="extra">
                    <el-button type="primary" @click="addItem">添加学校</el-button>
                </div>
            </div>
        </template>
